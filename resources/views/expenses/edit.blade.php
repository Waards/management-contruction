                ...
                <form method="POST" action="{{ route('expenses.update', $expense) }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <x-text-input id="description" name="description" type="text" class="mt-1 block w-full" value="{{ $expense->description }}" required />
                        </div>

                        <div>
                            <x-input-label for="amount" value="Amount" />
                            <x-text-input id="amount" name="amount" type="number" step="0.01" class="mt-1 block w-full" value="{{ $expense->amount }}" required />
                        </div>

                        <div>
                            <x-input-label for="type" value="Type" />
                            <select id="type" name="type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                <option value="labor" {{ $expense->type === 'labor' ? 'selected' : '' }}>Labor</option>
                                <option value="material" {{ $expense->type === 'material' ? 'selected' : '' }}>Material</option>
                                <option value="equipment" {{ $expense->type === 'equipment' ? 'selected' : '' }}>Equipment</option>
                                <option value="transport" {{ $expense->type === 'transport' ? 'selected' : '' }}>Transport</option>
                        ...
                        <div>
                            ...
                            <x-text-input id="expense_date" name="expense_date" type="date" class="mt-1 block w-full" value="{{ $expense->expense_date }}" required />
                        </div>
                        ...
                        <div>
                            ...
                            <select id="worker_id" name="worker_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                ...
                                @foreach($workers as $worker)
                                    <option value="{{ $worker->id }}" {{ $expense->worker_id === $worker->id ? 'selected' : '' }}>{{ $worker->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
